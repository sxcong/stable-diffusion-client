...
void PhotoDir::setCurrentFile(QString& fileName)
{
    for (const Photo *photo : _photos)
    {
        if (QDir::fromNativeSeparators(photo->fullPath()) == QDir::fromNativeSeparators(fileName))
        //if (photo->fullPath() == fileName)
        {
             setCurrent((Photo *)photo);
             break;
        }
    }
}
