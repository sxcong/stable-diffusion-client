...
void BorderPanel::appearNow()
{
    ...
    //emit aboutToAppear();
   // setPos(640,50 );
   // setPos(_sensitiveBorder->rect().left(),_sensitiveBorder->rect().top());
    show();
    _active = true;
}
...
{
    qDebug()<<"BorderPanel::maybeDisappear()";
    if ( ! _sticky )
	disappearAnimated();
}
...
QPointF BorderPanel::activePos()
{
    ...
    qDebug()<<"activePos panelX, panelY"<<panelX<< panelY;
...
QPointF BorderPanel::inactivePos()
{
    ...
    qDebug()<<"inactivePos panelX, panelY"<<panelX<< panelY;
    return secondaryPos( QPointF( panelX, panelY ) );
}
...
QPointF BorderPanel::secondaryPos( const QPointF( primaryPos ) )
{
    ...
    qDebug()<<"_sensitiveBorder->rect()"<<_sensitiveBorder->rect()<<parentRect<<panelSize;
    ...
    panelX = parentRect.left();
    panelY = parentRect.top();
