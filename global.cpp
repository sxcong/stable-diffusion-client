...
void Global::Init()
{
    ...
    QString cfgpath = path + "/config/config.ini";
    if (QFile::exists(cfgpath))
    {
        ...
        if (setting.contains("m_nSinglePhasePos"))
        {
            //m_nSinglePhasePos = setting.value("m_nSinglePhasePos").toString().toInt();
        }

        if (setting.contains("m_nPhaseBeforPos"))
        {
           //m_nPhaseBeforPos = setting.value("m_nPhaseBeforPos").toString().toInt();
        }
        setting.endGroup();
    }
}
...
void Global::SDVersion()
{
    qDebug() << version_string().c_str();
}
...
void parse_args(int argc, const char** argv, SDCliParams& cli_params, SDContextParams& ctx_params, SDGenerationParams& gen_params) {
   ...
   /* if (!parse_options(argc, argv, options_vec)) {
    ...
    ctx_params.lora_model_dir  = argv[2];
    ctx_params.model_path  = argv[2];
    cli_params.output_path = "output1.png";
    gen_params.prompt          = argv[4];
}

int Global::testSD(const std::string& prompt, const std::string& modelPath, const std::string& outputImagePath)
{
   ...
   /* const char *argv[5];
    argv[0], "SDClient";
    argv[1] = "-m";
    argv[2] = modelPath.c_str();
    argv[3] = "-p";
    argv[4] = outputImagePath.c_str();
    */


    //parse_args(argc, argv, cli_params, ctx_params, gen_params);

    //ctx_params.lora_model_dir = "d:/2026/v1-5-pruned-emaonly.safetensors";//    argv[2];
    //ctx_params.model_path     = "d:/2026/v1-5-pruned-emaonly.safetensors";  //    argv[2];
    //cli_params.output_path    = "output1.png";
    //gen_params.prompt         = "a black-and-white Border Collie";// argv[4];


    ctx_params.lora_model_dir = modelPath;
    ctx_params.model_path     = modelPath;
    cli_params.output_path    = outputImagePath;
    gen_params.prompt         = "a black-and-white Border Collie";
